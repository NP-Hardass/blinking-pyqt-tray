...
if find_loader('PyQt4') is not None:
    try:
        from PyQt4.QtGui import QSystemTrayIcon, QMenu, QAction, QIcon, QWidget, QApplication, qApp
        from PyQt4.QtCore import QTimer, QProcess
    except:
        raise
elif find_loader('PyQt5') is not None:
    try:
        from PyQt5.QtGui import QIcon 
        from PyQt5.QtWidgets import QSystemTrayIcon, QMenu, QAction, QWidget, QApplication, qApp
        from PyQt5.QtCore import QTimer, QProcess
    except:
        raise
elif find_loader('PySide') is not None:
    try:
        from PySide.QtGui import QSystemTrayIcon, QMenu, QAction, QIcon, QWidget, QApplication, qApp
        from PySide.QtCore import QTimer, QProcess
    except:
        raise
else:
    raise Exception('No valid Python Qt bindings found.')
